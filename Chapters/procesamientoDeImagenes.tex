...
Una imagen satelital es una funci\'on $ f:(x,y,i) \longrightarrow \{0,...,2^{r}\} $. Cada $ (x,y,i) $ indica la posición (x,y) en a banda i, donde $ i \in \{1,...,k\} $, $ x \in \{0,...,m\} $ e $ y \in \{0,...,n\} $ para una matriz $ m \times n $, siendo $ k $ el numero de bandas y $ r $ la resoluci\'on radiom\'etrica en la imagen. Las im\'agenes satelitales son conocidos tambi\'en como raster \cite{vasquez2011mineria}.
  \begin{figure}[H]
  	\centering
...
La Figura \ref{fig:imagenMultiespectral} nos muestra los ejes de coordenadas espaciales $ (x,y) $ para cada plano que representan las bandas, pudiendo acceder a valor de la intensidad o nivel digital mediante el nivel digital  $ f(x,y,i) $.


\subsection{Histogramas}
Sea $\Psi:Z \longrightarrow  Z$ una funci\'on que determina la frecuencia del nivel digital $ n_{nd} \in \{0,...,2^{r}\} $ en una imagen.\\~\\
...
El \'algebra de mapas constituye un marco te\'orico en la mayor parte de las operaciones hechas con SIG a partir de raster. Pueden desarrollarse operaciones de muy diverso tipo que se clasifican \cite{tomlin1990map} en:
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Condicional (si condici\'on entonces instrucciones sino instrucciones)
		\end{itemize}
		La Figura \ref{fig:oploccond} nos muestra el proceso de operadores locales l\'ogico y condicional.
		  \begin{figure}[H]
		  	\centering
		...
		\begin{itemize}
			\item Filtrado de im\'agenes: es el conjunto de t\'ecnicas que se aplican a las im\'agenes digitales con el objetivo de mejorar la calidad o facilitar la b\'usqueda de informaci\'on. En la Figura \ref{fig:oplovec} podemos observar un filtrado correspondiente a la media.
			\item Operadores estad\'isticos: calculan un estad\'istico (media, desviaci\'on t\'ipica, m\'inimo, m\'aximo, etc.) a partir de los valores de todas los pixeles que forman la vecindad y lo adjudican al pixel central en la imagen de salida. 
...
En la Figura \ref{fig:intPolEcua} podemos observar las transformaciones con polinomios de primer y segundo orden.
 ...
 \end{equation} 
 	...
 	El valor de $ RMS \in [0,\infty] $ elegido por referencia para corregir una imagen debe ser aproximadamente $ 0.5 $, y en lo posible nunca superar la unidad \cite{guide1999erdas}. \\~\\
...
La interpolaci\'on de los valores radiom\'etricos es el traslado del nivel digital perteneciente a la imagen original a la imagen corregida espacialmente. La imagen original debe corresponderse con las coordenadas de la imagen corregida, por lo tanto, resulta necesario trasvasar de alguna forma los niveles digital originales a su nueva posici\'on. Pudi\'endose ser abordada por tres m\'etodos diferentes:
	\begin{itemize}
...
Sea $ f^{'}:(x,y,i)\longrightarrow \{0,...,2^{r}\} $ una funci\'on simple que corrige los pixeles o lineas perdidas en la coordenadas $ (x,y) $ de la banda $ i $, mediante estimaciones de la media entre los pixeles contiguos:

		\begin{equation}\label{ec:pixelPerdido}
		f^{'}(x,y,i) = \ulcorner (\dfrac{f(x-1,y,i) + f(x+1,y,i)}{2}) \urcorner
		\end{equation} 
Donde ($ \ulcorner \urcorner $) representa redondeo hacia arriba. No es recomendable utilizar los pixeles contiguos de la misma linea (eje $ y $) por que han sido captados por el mismo detector o banda que ha dado el fallo, por tanto son poco fiables.
...
La bandas de una imagen son de detectores diferentes y est\'an altamente correlacionadas, por lo que se hace una modificaci\'on a la ecuaci\'on \ref{ec:pixelPerdido} teniendo en cuenta la banda $ i_{*} $ de la imagen, donde $ i_{*} \in [1,k] $, pudiendo utilizarse el valor del pixel faltante en una banda diferente para mejorar la estimaci\'on:
		\begin{equation}
