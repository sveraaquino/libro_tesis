...
En la Figura \ref{fig:intPolEcua} podemos observar las transformaciones con polinomios de primer y segundo orden.
  ...
  \paragraph{Ajuste por m\'inimos cuadrados }\label{sec:minimoscuadrados}\mbox{}\\\mbox{}\\
Una relación funcional $ y = \alpha(x) $ entre $ x $ y $ y $, con $ \alpha $ desconocida y se conocen valores $ y_{u} $ que aproximan a $ \alpha(x_{u}) $, o sea:
 \begin{equation}
\alpha(x_u ) = y_u + \varepsilon _u
 \end{equation} 
Donde $ u \in \{1,..,\eta\}$, con $  \varepsilon _u $  desconocido.
El m\'etodo trata de recuperar la función $ \alpha $ a partir de los datos $ y_u $. Si $ \alpha $ es polin\'omica $ (\alpha(x) = p_{l}(x)) $, entonces, dados $ \eta+1 $ puntos $ (x_0, y_0), (x_1, y_1),..., (x_\eta, y_\eta) $ con $ x_0, x_1,..., x_\eta $ n\'umeros reales diferentes, se trata de encontrar un polinomio:
  \begin{equation}
  p_m (x) = a_0 + a_1 x + \ldots + a_l x^l
  \end{equation} 
  que mejor se ajuste a los datos y sea m\'inimo. En otras palabras:
  \begin{equation}
s(a_0 ,a_1 , \ldots ,a_l ) = \sum\limits_{u = 0}^\eta {\left( {p_m (x_u ) - y_u } \right)^2 } = \sum\limits_{u = 0}^\eta {\left( {a_0 + a_1 x_u + a_2 x_u^2 + \ldots + a_l x_u^l - y_k } \right)^2 }
  \end{equation}   
 Donde $ s $ es el conjunto de coeficientes del polinomio.
 \nomenclature[30]{$ p_m (x) $}{Polinomio con variable dependiente $ x $.}
 \nomenclature[30]{$ \alpha $}{Una funci\'on cualquiera.}
\nomenclature[30]{$ S $}{Conjunto de coeficientes del polinomio.}
