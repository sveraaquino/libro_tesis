		...
		\end{equation}	
				...
				\nomenclature[33]{$ \rho(x,y,i) $}{Nivel digital en la posici\'on $ (x,y) $ de $ \rho $ en la banda $ i $.}
		    		\item \textbf{Convoluci\'on c\'ubica:} es similar a la interporlaci\'on bilineal pero considera niveles digitales de los 16 pixeles m\'as pr\'oximos. El efecto visual es m\'as correcto, pero supone un volumen de c\'alculo mucho m\'as elevado. \\~\\
		    				    \begin{figure}[H]
		    				    	\centering
		    				    	\includegraphics[width=0.7	\textwidth]{./Figures/cap3/convolucionCubica_matriz.png}
					...
					Donde la funci\'on $ \beta:(q) \longrightarrow [-\infty,\infty] $, determina los coeficientes de $ \varphi(x,y,i) $ en base a un coeficiente $ q $ definida por $ d(p_{x,y,i},p'_{x,y,i}) $:
		    				    		...
		    				    		\nomenclature[37]{$ d(p_{x,y,i},p'_{x,y,i}) $}{ Distancia euclidiana entre un pixel $ p_{x,y,i} $ y $ p'_{x,y,i} $.}		    		
		    				    		\nomenclature[38]{$ \beta $}{ Funci\'on que determina los coeficientes de $ \varphi$.}
		    				    		\nomenclature[40]{$ q $}{ Coeficiente definido por $ d(p_{x,y,i},p'_{x,y,i}) $.}
