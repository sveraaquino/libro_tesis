...
\lhead{Cap\'itulo 4. \emph{Materiales y Metodolog\'ia}} % This is for the header on each page - perhaps a shortened title
...
%															\algrestore{myalg2}
  	...
  	\end{algorithmic}
  \end{algorithm}
\\  Cada valor de pixel, de las bandas en cada imagen satelital, es utilizado en el c\'alculo si no representa un cambio en la m\'ascara de cambio $ MC $. El proceso de iteraci\'on finaliza retornando una imagen de p\'erdida $ MP $ seg\'un la ecuaci\'on \ref{ec:umbMetodoMP}.\\~\\
...
\subsubsection{M\'ascara de P\'erdida Forestal}
...
\end{equation}
\nomenclature[59]{$ (m, h)$}{Coeficiente en la ecuaci\'on de estimaci\'on de carbono.}
...
%										\algrestore{myalg}
		...
		\State 
		\State \Comment{Se determina la m\'ascara de p\'erdida seg\'un la ecuaci\'on \ref{ec:umbMetodoMP}}
		\State $MP=\textsc{mascaraPerdida}(I_{c})$
		\Statex		
		\State \Comment{Se determina la m\'ascara de p\'erdida forestal intersectando $ MC $ con $ MV $}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6	\textwidth]{./Figures/cap4/ejemplo_entrada.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8	\textwidth]{./Figures/cap4/ejemplo_iteracion_1.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8	\textwidth]{./Figures/cap4/ejemplo_iteracion_3.png}
