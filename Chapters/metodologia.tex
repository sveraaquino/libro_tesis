...
En las distintas bandas de una imagen satelital se observan un comportamiento con distribucion normal en sus histogramas, por lo que teniendo una variable tipificada $(Z)$ es posible definirlos en una distribuci\'on normal est\'andar del tipo $ N(0,1) $ seg\'un la expresión:
	...
	Z=\dfrac{ND-\mu}{\sigma}
	\end{equation}
\nomenclature[26]{$ Z $}{Variable tipificada.}	
	...
	\begin{equation}
	\dfrac{ND_{1}-\mu_{1}}{\sigma_{1}}=\dfrac{ND_{2}-\mu_{2}}{\sigma_{2}}
	\end{equation}
	Para su aplicaci\'on pr\'actica, se puede transformar el nivel digital $ (ND) $ perteneciente a la imagen 1 $ (ND_{1}) $, para que se asemeje al $ ND_{2} $ de la imagen 2, expresi\'on:
	\begin{equation}
	ND_{1}^{'}=\mu_{2}+\dfrac{\sigma_{2}}{\sigma_{1}}\cdot(ND_{1}-\mu_{1})
	\end{equation}
	As\'i se puede definir una relaci\'on lineal entre las dos distribuciones; aplicando una normalizaci\'on radiom\'etrica estad\'istica, los coeficientes de la transformaci\'on $ m_{1,2} $ y $ h_{1,2} $ se definen seg\'un se indica en la expresi\'on:
	
			\begin{equation}
			h_{1,2}=\mu_{2}-\dfrac{\sigma_{2}}{\sigma_{1}}\cdot\mu_{1}
			\end{equation}
					\begin{equation}
					m_{1,2}=\dfrac{\sigma_{2}}{\sigma_{1}}
					\end{equation}
		\begin{equation}
				ND_{1}^{'}=m_{1,2}\cdot ND_{1}+h_{1,2}
		\end{equation}
...
El umbral discriminatorio para la vegetaci\'on es determinado a partir de una constante calculada por un previo estudio del comportamiento espectral de la cobertura vegetal y las im\'agenes NDVI \ref{subsec:umbralVegetacion}. Dicha variable corresponde a la desviaci\'on $(\sigma_{c} = 0.0658242733)  $ observada en el an\'alisis hecho por cruzamientos de datos entre im\'agenes VCF y Landsat. La ecuaci\'on del umbral $ U_{NDVI} $ tiene la siguiente expresi\'on:
		...
		\end{equation}
\nomenclature[27]{$ \sigma_{c} $}{Desviaci\'on determinada como constante para la umbralizaci\'on de la cobertura vegetal.}
...
\nomenclature[28]{$ r^{2}$}{Coeficiente de determinaci\'on.}
\begin{table}[H]
	\centering
	\begin{tabular}{|l|l|}
		\hline
...
La siguiente expresi\'on define la ecuaci\'on hallada:
		...
		\end{equation}
\nomenclature[29]{$ C_{(x,y)}$}{Toneladas de carbono por hect\'area en las coordenadas $ x $ e $ y $.}
...
Restando el carbono obtenido entre cada fecha, ecuación \ref{e:fecha2} y \ref{e:fecha1}, tendr\'iamos:
		...
		\end{equation}		
				...
				\end{equation}		
\nomenclature[30]{$PC_{(x,y)}$}{Toneladas de carbono por hect\'area perdidos en las coordenadas $ x $ e $ y $.}
Siendo $ PC_{(x,y)}$ Toneladas de carbono por hect\'area perdidos. Remplazando \ref{e:indiceCambio} y \ref{e:perdidaCar} en \ref{e:restaCar}, la ecuaci\'on de regresi\'on final quedar\'ia:
		\begin{equation}
